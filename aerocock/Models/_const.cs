...
namespace aerocock.Models
{
    public class _const
    {
        ...
        public static double metrs { get { return 0.125; } set { metrs = value; } }

        public int fps,
            coopMaxAttempts, coopScoreMultiplier, coopWinScore, coopMaxTime, vsMaxTime,
            vsMaxScore, vsMaxWins, wordsGameTime;

        public double width, height, /*offset,*///real proportions 
            minVelocity, maxVelocity, multiplier, radius, gameballradius,
            fieldVerticalOffset, coopPointHalfWidth, 
            lifeTimeMs, newPointsMs, startBlinkMs;

        public WMPLib.WindowsMediaPlayer soundBackground;

        public _const()
        {
            Matrix = aerocock.Models.Instructions.SetMatrix(Matrix, _const.metrs * 100);
            ...
            coopScoreMultiplier = 5; //множитель числа очков при выбивании очкового шара (придумано для вывода более красивых цифр на экран)
            ...
            startBlinkMs = 3200;     //через сколько появившийся очко-шар начнёт мерцать (говорить, что скоро исчезнет) [миллисекунды]
